...
def matching_index(
    ...
    assert mode in [
        ...
        "in",
        "out",
    ], f"Mode '{mode}' is not supported. Must be one of 'all', 'in', or 'out'."
    ...
    if mode == "in":
        ...
        if divisor == "mean":
            ...
            matching_indices = (
                modified_adjacency_matrix.T @ modified_adjacency_matrix
            ) / denominator
            ...
            denominator = (
                torch.max(
                    modified_adjacency_matrix.unsqueeze(1),
                    modified_adjacency_matrix.unsqueeze(2),
                ).sum(dim=0)
                - modified_adjacency_matrix
                - modified_adjacency_matrix.T
            )
        ...
        if divisor == "mean":
            ...
            combined_adjacency_matrix = (
                modified_adjacency_matrix + modified_adjacency_matrix.T
            )
            ...
            denominator = (
                node_strengths.unsqueeze(0)
                + node_strengths.unsqueeze(1)
                - 2 * combined_adjacency_matrix
            ) / 2
            denominator[denominator == 0] = 1  # Avoid division by zero

            # Compute the matching index
            matching_indices = (
                combined_adjacency_matrix.T @ combined_adjacency_matrix
            ) / denominator
            matching_indices.fill_diagonal_(0)
            ...
            denominator = (
                torch.max(
                    combined_adjacency_matrix.unsqueeze(1),
                    combined_adjacency_matrix.unsqueeze(2),
                ).sum(dim=0)
                - combined_adjacency_matrix
                - combined_adjacency_matrix.T
            )
            # Set the denominator to be 1 whenever it is zero to avoid division by zero
            denominator[denominator == 0] = 1
