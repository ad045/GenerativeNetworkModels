...
class SweepConfig:
    ...
    num_random_samples: Optional[int] = 10, 
    ...
    def __iter__(self) -> Iterator[RunConfig]:
        ...
        If `method` is "grid", it yields every possible combination of parameters.
        If `method` is "random", it yields `num_random_samples` random combinations.
        ...
        seed_adj_list = self.seed_adjacency_matrix or [None]
        distance_list = self.distance_matrix or [None]
        heterochronous_list = self.heterochronous_matrix or [None]
        seed_weight_list = self.seed_weight_matrix or [None]
        ...
        # random sampling
            ...
            if self.num_random_samples is None:
                raise ValueError(
                    "num_random_samples must be specified when method is 'random'"
                )

            # Convert iterable parameter spaces to lists to enable random.choice
            binary_params_list = list(self.binary_sweep_parameters)
            weighted_params_list = (
                list(self.weighted_sweep_parameters)
                if self.weighted_sweep_parameters is not None
                else [None]
            )
            
            # Yield the specified number of random configurations
            for _ in range(self.num_random_samples):
                binary_params = random.choice(binary_params_list)
                seed_adj = random.choice(seed_adj_list)
                distance_matrix = random.choice(distance_list)
                weighted_parameters = random.choice(weighted_params_list)
                seed_weights = random.choice(seed_weight_list)
                heterochronous_matrix = random.choice(heterochronous_list)
