...
sweep_configuration = {
    ...
    "parameters": {
        ...
        "distance_relationship_type": {
            "value": "exponential"
        },  # {"values": ["powerlaw", "exponential"]},
        "matching_relationship_type": {
            "value": "exponential"
        },  # {"values": ["powerlaw", "exponential"]},
        "alpha": {"min": 0, "max": 1},
        "optimisation_criterion": {
            "value": "distance_weighted_communicability"
        },  # {"values": ["distance_weighted_communicability", "weighted_distance"]},
...
def main():
    ...
    gnm.train_loop(
        ...
        binary_updates_per_iteration=binary_updates_per_iteration,
    ...
    real_clustering_coefficients_list = list(
        nx.clustering(Greal, weight="weight").values()
    )
    real_betweenness_centrality_list = list(
        nx.betweenness_centrality(Greal, weight="weight").values()
    )
    ...
    degree_KS_statistic = ks_2samp(
        real_weighted_degree_list, weighted_degree_list
    ).statistic
