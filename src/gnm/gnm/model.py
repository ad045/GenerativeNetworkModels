...
class BinaryGenerativeParameters:
    ...
    generative_rule: GenerativeRule
    num_iterations: int
    prob_offset: float = 1e-8 # previously 1e-6
...
class GenerativeNetworkModel:
    ...
    **Weight Optimisation Phase (Optional):**
    ...
    def binary_update(
        ...
        # After calculating factors, clamp to reasonable ranges
        # (THIS IS A PROTOTYPE - CHECK FOR SENSIBILITY!) TODO 
        # This is a change that was not in the original code (HACK)
        matching_factor = torch.clamp(matching_factor, min=1e-20, max=1e20)
        heterochronous_factor = torch.clamp(heterochronous_factor, min=1e-20, max=1e20)
        ...
        # Check for numerical issues before normalization
        # (THIS IS A PROTOTYPE - CHECK FOR SENSIBILITY!) TODO 
        if self.verbose: 
            if torch.any(torch.isnan(unnormalised_wiring_probabilities)) or torch.any(torch.isinf(unnormalised_wiring_probabilities)):
                print(f"Warning: Numerical instability detected. gamma={self.binary_parameters.gamma}, lambdah={self.binary_parameters.lambdah}")
                # TODO: Here is the warning regarding "Warning: Numerical instability detected..."
                # Maybe add fallback strategy here? 
        ...
        if not torch.isfinite(flattened_probs).all() or (flattened_probs < 0).any():
            raise RuntimeError("Probability tensor contains either `inf`, `nan` or element < 0")
        # print("Sum probs:", flattened_probs.sum(dim=-1), ". Should be 1.")
