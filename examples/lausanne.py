                    ...
                    for lambdah in lambdah_values:
                        for alpha in alpha_values:
                            ...
                            for run in range(num_runs):
                                ...
                                print(
                                    f"Progress: {progress_percentage:.2f}% ({current_model}/{total_models})"
                                )
                                ...
                                real_degree_list = np.array(
                                    [degree for node, degree in Greal_bin.degree()]
                                )
                                real_clustering_coefficients_list = np.array(
                                    list(nx.clustering(Greal_bin).values())
                                )
                                real_betweenness_centrality_list = np.array(
                                    list(
                                        nx.betweenness_centrality(
                                            Greal_bin, normalized=False
                                        ).values()
                                    )
                                )
                                ...
                                real_connected_distances = distance_matrix.numpy()[
                                    real_connected_indices
                                ]

                                # Compute binary network metrics for the synthetic network
                                degree_list = np.array(
                                    [degree for node, degree in G_bin.degree()]
                                )
                                clustering_coefficients_list = np.array(
                                    list(nx.clustering(G_bin).values())
                                )
                                betweenness_centrality_list = np.array(
                                    list(
                                        nx.betweenness_centrality(
                                            G_bin, normalized=False
                                        ).values()
                                    )
                                )
                                ...
                                connected_distances = distance_matrix.numpy()[
                                    connected_indices
                                ]

                                # --- Compute Topological Energy (KS Statistics) ---
                                # Compute KS statistics
                                degree_KS_statistic = ks_2samp(
                                    real_degree_list, degree_list
                                ).statistic
                                clustering_KS_statistic = ks_2samp(
                                    real_clustering_coefficients_list,
                                    clustering_coefficients_list,
                                ).statistic
                                betweenness_KS_statistic = ks_2samp(
                                    real_betweenness_centrality_list,
                                ...
                                real_clustering_coefficients_list = W.dot(
                                    real_clustering_coefficients_list
                                )
                                clustering_coefficients_list = W.dot(
                                    clustering_coefficients_list
                                )

                                real_betweenness_centrality_list = W.dot(
                                    real_betweenness_centrality_list
                                )
                                betweenness_centrality_list = W.dot(
                                    betweenness_centrality_list
                                )
                                ...
                                degree_correlation = compute_correlation(
                                    real_degree_list, degree_list
                                )
                                ...
                                betweenness_error = (
                                    1 - (betweenness_correlation + 1) / 2
                                )

                                # Compute the maximum error statistic for topography
                                topography_energy = max(
                                    [degree_error, clustering_error, betweenness_error]
                                )

                                # --- Combine Energies ---
                                total_energy = (
                                    beta * topology_energy
                                    + (1 - beta) * topography_energy
                                )
                            ...
                            avg_clustering_correlations = np.mean(
                                clustering_correlations
                            )
                            avg_betweenness_correlations = np.mean(
                                betweenness_correlations
                            )
...
beta = 0.5
results_df["total_energy"] = (
    beta * results_df["topology_energy"] + (1 - beta) * results_df["topography_energy"]
)
...
eta_gamma_pivot = eta_gamma_fixed_df.pivot(
    index="eta", columns="gamma", values=which_energy
)

plt.figure(figsize=(10, 8))
sns.heatmap(eta_gamma_pivot, annot=True, cmap="viridis")
plt.title(
    f"Energy Heatmap for eta vs gamma (lambdah={best_lambdah}, alpha={best_alpha})"
)
plt.xlabel("gamma")
plt.ylabel("eta")
plt.show()
...
eta_lambdah_pivot = eta_lambdah_fixed_df.pivot(
    index="eta", columns="lambdah", values=which_energy
)
...
gamma_alpha_pivot = gamma_alpha_fixed_df.pivot(
    index="gamma", columns="alpha", values=which_energy
)
...
for t in range(n_steps):
    ax = axes[t]
    Ht = heterochronous_matrix[
        :, :, int(np.round(num_seed_edges / n_steps * t))
    ].numpy()
    node_values = (
        Ht.diagonal()
    )  # Use the diagonal elements to represent nodal values for coloring
...
clustering_correlation = pearsonr(
...
betweenness_correlation = pearsonr(
...
plt.scatter(coordinates[:, 2], coordinates[:, 0], coordinates[:, 1], c=real_degree_list)
...
for beta in beta_values:
    results_df["total_energy"] = (
        beta * results_df["topology_energy"]
        + (1 - beta) * results_df["topography_energy"]
    )
...
plt.plot(
    best_params_df["beta"],
    best_params_df["total_energy"],
    label="Total Energy",
    marker="o",
)
plt.plot(
    best_params_df["beta"],
    best_params_df["topology_energy"],
    label="Topology Energy",
    marker="s",
)
plt.plot(
    best_params_df["beta"],
    best_params_df["topography_energy"],
    label="Topography Energy",
    marker="^",
)
...
def pareto_frontier(data, objectives):
    ...
    sorted_data = data.sort_values(by=objectives[0], ascending=True).reset_index(
        drop=True
    )
...
pareto_data["topology_energy"] = (
    pareto_data["topology_energy"] - pareto_data["topology_energy"].min()
) / (pareto_data["topology_energy"].max() - pareto_data["topology_energy"].min())

pareto_data["topography_energy"] = (
    pareto_data["topography_energy"] - pareto_data["topography_energy"].min()
...
pareto_front = pareto_frontier(
    pareto_data, ["topology_energy_norm", "topography_energy_norm"]
)
...
plt.plot(
    pareto_front["topology_energy"],
    pareto_front["topography_energy"],
    color="red",
    marker="o",
    label="Pareto Front",
)
