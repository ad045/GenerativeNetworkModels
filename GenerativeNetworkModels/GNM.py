...
class GenerativeNetworkModel:
    ...
    def __init__(
        ...
        if not torch.all(
            (self.seed_adjacency_matrix == 0) | (self.seed_adjacency_matrix == 1)
        ):
            raise ValueError(
                f"The seed_adjacency_matrix must be binary (contain only 0s and 1s). Recieved {self.seed_adjacency_matrix}."
            )
        # Check that the distance matrix is symmetric.
        if not torch.allclose(self.distance_matrix, self.distance_matrix.T):
            raise ValueError(
                f"The distance matrix must be symmetric. Recieved {self.distance_matrix}."
            )
        # Check that the distance matrix is non-negative.
        if torch.any(self.distance_matrix < 0):
            raise ValueError(
                f"The distance matrix must be non-negative. Recieved {self.distance_matrix}."
            )
        ...
        if distance_relationship_type not in ["powerlaw", "exponential"]:
            raise NotImplementedError(
                f"Distance relationship type '{distance_relationship_type}' is not supported for the binary GNM."
            )
        if matching_relationship_type not in ["powerlaw", "exponential"]:
            raise NotImplementedError(
                f"Matching relationship type '{matching_relationship_type}' is not supported for the binary GNM."
            )
        ...
        if alpha is not None:
            ...
            if seed_weight_matrix is None:
                ...
                if not torch.allclose(seed_weight_matrix, seed_weight_matrix.T):
                    raise ValueError(
                        f"The seed_weight_matrix must be symmetric. Recieved {seed_weight_matrix}."
                    )
                # Check that the seed_weight_matrix is non-negative.
                if torch.any(seed_weight_matrix < 0):
                    raise ValueError(
                        f"The seed_weight_matrix must be non-negative. Recieved {seed_weight_matrix}."
                    )
                # Check that the seed weight matrix has support only where the seed adjacency matrix is non-zero.
                if torch.any(
                    (self.seed_adjacency_matrix == 0) & (seed_weight_matrix != 0)
                ):
                    raise ValueError(
                        f"The seed_weight_matrix must have support only where the seed_adjacency_matrix is non-zero. Recieved seed adjacency matrix {self.seed_adjacency_matrix} and seed weight matrix {seed_weight_matrix}."
                    )
            ...
            if optimisation_normalisation is None:
                print(
                    "Optimisation normalisation was unspecified. Defaulting to False."
                )
            ...
            if self.optimisation_criterion_name == "distance_weighted_communicability":
                assert (
                    "omega" in optimisation_criterion_kwargs
                ), "The 'omega' parameter must be specified for the 'distance_weighted_communicability' optimisation criterion."
                self.optimisation_criterion = DistanceWeightedCommunicability(
                    normalisation=self.optimisation_normalisation,
                    distance_matrix=self.distance_matrix,
                    **optimisation_criterion_kwargs,
                )
            elif self.optimisation_criterion_name == "weighted_distance":
                self.optimisation_criterion = WeightedDistance(
                    normalisation=self.optimisation_normalisation,
                    distance_matrix=self.distance_matrix,
                    **optimisation_criterion_kwargs,
                )
                ...
                raise NotImplementedError(
                    "The specified optimisation criterion is not yet supported."
                )
            ...
            if weight_lower_bound is not None:
                if weight_lower_bound < 0.0:
                    raise ValueError(
                        f"The weight_lower_bound must be non-negative. Recieved {self.weight_lower_bound}."
                    )
            ...
            if weight_upper_bound is not None:
                if weight_upper_bound < 0.0:
                    raise ValueError(
                        f"The weight_upper_bound must be non-negative. Recieved {self.weight_upper_bound}."
                    )
            ...
            self.optimiser = optim.SGD(
                [self.weight_matrix], lr=self.alpha, maximize=maximise_criterion
            )
